...
import { progressSender, text, tool, withData, type Server } from "../result.js";
...
type ScanResult = {
  ...
  shotsRoot: string;
  pickedLocale?: string;
  locales: { code: string; frames: { no: number; headline: string }[] }[];
  images: ImageRef[];
  localeImages?: Record<string, ImageRef[]>;
};
...
type ImportResult = {
  ...
  available: { code: string; frames: number }[];
  locales: { code: string; slides: number; matched: number; matched2: number; unmatched: string[] }[];
};
...
function applyPatch(
  ...
  if (patch.badge !== undefined) slide.badge = patch.badge || undefined;
  if (patch.layout !== undefined) slide.layout = patch.layout;
  ...
  if (patch.ornamentText !== undefined) {
    if (patch.ornamentText) slide.ornament = { kind: "laurel", text: patch.ornamentText };
    else delete slide.ornament;
  }
  if (patch.textOffset !== undefined) slide.textOffset = patch.textOffset;
  if (patch.deviceOffset !== undefined) slide.deviceOffset = patch.deviceOffset;
  if (patch.screenshotId !== undefined) {
    const url = shotById.get(patch.screenshotId);
    if (!url) {
      throw new VitrinError(
        `Projede böyle bir ekran görüntüsü yok: ${patch.screenshotId}`,
        "vitrin_open_project ile mevcut ekranları gör."
      );
    }
    slide.screenshot = url;
  }
  if (patch.clearBackground) delete slide.background;
  if (patch.clearDevice) delete slide.device;
}
...
export function registerContent(server: Server): void {
  tool(
    server,
    "vitrin_scan_folder",
    {
      title: "Klasörü tara",
      description:
        "Bir pazarlama klasöründeki .md kare dosyalarını ve ekran görüntülerini tarar. " +
    ...
    async ({ folder, shotsRoot }) => {
      const r = await api<ScanResult>("/api/ingest", { path: folder, shotsRoot: shotsRoot || undefined });
  ...
  tool(
    ...
    "vitrin_import",
    {
      ...
      description:
        "Taranan klasörü projeye yazar: kareler slayta, EYEBROW rozete, LAUREL çelenge, " +
    ...
    async (args) => {
      const r = await api<ImportResult>("/api/sync/import", args, { timeoutMs: 300_000 });
      const lines = [
        `${r.app} · ${r.folder}`,
        ...
        ...r.locales.map(
          (l) =>
            `${l.code.padEnd(8)} ${l.slides} slayt · ekran eşleşen ${l.matched}/${l.slides}` +
      ...
      return withData(text(...lines), r as unknown as Record<string, unknown>);
    }
  );
  ...
  tool(
    ...
    "vitrin_art_direct",
    {
      ...
      description:
        "Projedeki ekran görüntülerine BAKARAK n slaytlık anlatı kurar (başlık, alt satır, " +
    ...
    async ({ projectId, locale, n, keywords, brief, themeIds }, ctx) => {
      ...
      if (!p.screenshots.length) {
        throw new VitrinError(
          "Projede hiç ekran görüntüsü yok",
          "Önce vitrin_import ile ekranları içeri al."
        );
      }
      ...
      const r = await apiLlm<{ slides: { headline: string; sub: string; badge?: string; themeId: string; layout: string; screenshotId: string }[] }>(
        "/api/director",
        {
          app: p.app,
          brief: p.brief || "",
      ...
      p.sets[loc] = {
        ...
        slides: r.slides.map((d, i) => {
          ...
          if (d.badge) slide.badge = d.badge;
          const url = byId.get(d.screenshotId);
          if (url) slide.screenshot = url;
          return slide;
        }),
      };
      p.activeLocale = loc;
      ...
      await saveProject(p);
      return withData(
        text(`${loc} · ${r.slides.length} slayt kurgulandı`, "", summarizeSet(p.sets[loc])),
        { locale: loc, slides: r.slides.length }
      );
    }
  );
  ...
  tool(
    ...
    "vitrin_translate",
    {
      title: "Dillere çevir",
      description:
        "Kaynak dilin başlık/alt satırlarını TEK istekte hedef dillere çevirir (/api/copy " +
    ...
    async ({ projectId, fromLocale, langs }, ctx) => {
      ...
      if (!src.slides.length) throw new VitrinError(`${from} setinde slayt yok`, "Önce içe aktar.");

      const targets = langs.map((l) => l.trim()).filter((l) => l && l !== from);
      if (!targets.length) throw new VitrinError("Hedef dil listesi boş", "Kaynak dille aynı olamaz.");

      progress(0, targets.length, "çeviri isteniyor");
      const texts = src.slides.map((s) => ({ head: s.headline, sub: s.sub ?? "" }));
      const r = await apiLlm<{ packs: Record<string, { head: string; sub: string }[]> }>(
        "/api/copy",
        { action: "localize", texts, langs: targets }
      );
      ...
      targets.forEach((code, i) => {
        ...
        if (pack.length < src.slides.length) partial.push(code);
        ...
        if (existing) {
          ...
          p.sets[code] = {
            ...
            slides: src.slides.map((s, j) => {
              const copy: Slide = structuredClone(s);
              copy.id = uid();
              const t = pack[j];
              if (t) {
      ...
      return withData(
        text(
          `${from} → ${done.join(", ")} · ${src.slides.length} slayt`,
          partial.length ? `EKSİK paket gelen diller: ${partial.join(", ")} (kalan slaytlar çevrilmedi işaretli)` : "",
          `Projedeki diller: ${Object.keys(p.sets).join(", ")}`,
          "",
          "Tasarımı da hizalamak istersen: vitrin_sync_design"
        ),
        { from, translated: done, partial, locales: Object.keys(p.sets) }
      );
    }
  );
  ...
  tool(
    ...
    "vitrin_edit_slide",
    {
      ...
      description:
        "Tek bir slaytın metnini/tasarımını değiştirir. Projede tasarım senkronu açıksa " +
  ...
  tool(
    server,
    "vitrin_reorder_slides",
    {
      title: "Slaytları sırala",
      description:
        "Seçili dildeki slaytları verilen sıraya dizer. order, TÜM indeksleri bir kez " +
    ...
    async ({ projectId, locale, order, syncOtherLocales }) => {
      ...
      await mutateProject(projectId, (p) => {
        ...
        if (!ok) {
          throw new VitrinError(
            `order ${n} slaytın hepsini bir kez içermeli (gelen: ${order.join(",")})`,
            "Örn. 5 slayt için [0,2,1,4,3]."
          );
        }
  ...
  tool(
    ...
    "vitrin_add_slide",
    {
      title: "Slayt ekle",
      description:
        "Seçili dile yeni slayt ekler. Kaynak verilmezse son slaydın tasarımı kopyalanır " +
    ...
    async ({ projectId, locale, at, copyFrom, headline, sub, screenshotId }) => {
      ...
      await mutateProject(projectId, (p) => {
        ...
        if (screenshotId) {
          ...
          if (!url) throw new VitrinError(`Ekran görüntüsü yok: ${screenshotId}`, "");
          slide.screenshot = url;
        }
