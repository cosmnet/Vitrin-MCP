...
function renderFrame(f: Frame): string {
  ...
  if (f.badge) out.push(`EYEBROW: ${oneLine(f.badge)}`);
  if (f.laurel) out.push(`LAUREL: ${oneLine(f.laurel)}`);
  out.push(`HEADLINE: ${oneLine(f.headline)}`);
  out.push(`CHARS: ${oneLine(f.headline).length}`);
  if (f.sub) out.push(`SUB: ${oneLine(f.sub)}`);
  if (f.screen) out.push(`SCREEN: ${oneLine(f.screen)}`);
  if (f.screen2) out.push(`SCREEN2: ${oneLine(f.screen2)}`);
  if (f.layout) out.push(`LAYOUT: ${oneLine(f.layout)}`);
  return out.join("\n");
}

export function renderFramesMd(locale: string, frames: Frame[], note?: string): string {
  const head = [`# Vitrin kareleri · ${locale}`];
  if (note) head.push(`<!-- ${oneLine(note)} -->`);
  head.push("");
...
async function validate(locale: string, frames: Frame[]): Promise<string[]> {
  const problems: string[] = [];

  if (!LOCALE_RE.test(locale)) {
    problems.push(
      `Dil kodu geçersiz: "${locale}". Dosya adı dil kodu olur — "tr", "en-US", "zh-Hans" gibi.`
    );
  }

  if (!frames.length) problems.push("Hiç kare verilmedi.");
  ...
  for (const [no, n] of seen) {
    if (n > 1) problems.push(`Kare numarası ${no} ${n} kez geçiyor — numaralar tekil olmalı.`);
  }

  // Yerleşim id'leri stüdyodan; stüdyo kapalıysa bu kontrol atlanır.
  const used = [...new Set(frames.map((f) => f.layout).filter(Boolean) as string[])];
  if (used.length) {
    try {
...
export function registerMd(server: Server): void {
  tool(
    server,
    "vitrin_write_frames",
    {
      ...
      description:
        "Bir dilin kare MD dosyasını KANONİK biçimde yazar (<folder>/<locale>.md). " +
    ...
    async ({ folder, locale, frames, note, overwrite }) => {
      ...
      try {
        ...
        if (code === "EEXIST") {
          throw new VitrinError(
            `Dosya zaten var: ${file}`,
            "Üzerine yazmak istiyorsan overwrite=true ver."
          );
        }
        if (code === "ENOENT") {
      ...
      return withData(
        text(
          `Yazıldı: ${file}`,
          `${sorted.length} kare (${split} çift ekran)`,
          noHint
            ? `UYARI: ${noHint} karede SCREEN ipucu yok — eşleşme yalnız kare numarasına kalır.`
            : "Her karede SCREEN ipucu var.",
          "Doğrulamak için: vitrin_scan_folder"
        ),
        { file, locale, frames: sorted.length, split, withoutScreenHint: noHint }
      );
    }
  );
}
