...
import { text, tool, withData, type Server } from "../result.js";
import { ensureServer, health } from "../studio.js";

export function registerCore(server: Server): void {
  tool(
    server,
    "vitrin_status",
    {
      title: "Vitrin durumu",
      description:
        "Vitrin Studio dev sunucusunun ayakta olup olmadığını söyler, gerekirse başlatır; " +
    ...
    async () => {
      ...
      if (!before) await ensureServer();
      const rows = await listProjects();
      const cat = await catalog();
      return withData(
        text(
          `Dev sunucu: AYAKTA — ${BASE_URL}${before ? "" : " (bu çağrıda başlatıldı)"}`,
          `Klasör: ${VITRIN_DIR}`,
          `Tarayıcı: ${browserReady() ? `açık (WebGL: ${glMode()})` : "kapalı (ilk render'da açılır)"}`,
          `Proje: ${rows.length} · Tema: ${cat.themes.length} · Şablon: ${cat.templates.length} · Yerleşim: ${cat.layouts.length}`,
          "",
          summarizeList(rows)
        ),
        {
  ...
  tool(
    server,
    "vitrin_create_project",
    {
      title: "Yeni proje",
      description:
        "Boş bir Vitrin projesi oluşturur. Slaytlar sonra vitrin_import (MD klasöründen) " +
      ...
      inputSchema: z.object({
        ...
        format: z
          ...
          .optional()
          .describe(
            'Proje formatı, varsayılan "iphone". iPad seti AYRI projedir: tuval 3:4 ' +
    ...
    async ({ app, brief, locale, format }) => {
      ...
      return withData(
        text(`Proje oluşturuldu: ${project.app} · id=${project.id} · dil=${loc} · ${fmt} · 0 slayt`),
        { id: project.id, app: project.app, locale: loc, format: fmt }
      );
    }
  );
  ...
  tool(
    ...
    "vitrin_open_project",
    {
      ...
      description:
        "Projenin ÖZETİNİ döner: diller, slayt sayıları ve seçili dilin slayt tablosu " +
  ...
  tool(
    server,
    "vitrin_duplicate_project",
    {
      title: "Projeyi çoğalt",
      description:
        "Projeyi yeni id ile kopyalar; `format` verilirse o formata geçirir. " +
    ...
    async ({ projectId, app, format }) => {
      ...
      copy.format = fmt;
      for (const set of Object.values(copy.sets)) set.app = name;
  ...
  tool(
    server,
    "vitrin_delete_project",
    {
      title: "Projeyi sil",
      description:
        "Projeyi kalıcı olarak siler. Geri alınamaz — bu yüzden confirm alanına projenin " +
    ...
    async ({ projectId, confirm }) => {
      ...
      if (confirm.trim() !== p.app.trim()) {
        throw new VitrinError(
          `Onay eşleşmedi: "${confirm}" ≠ "${p.app}"`,
          "Silmek için confirm alanına projenin app adını aynen yaz."
        );
      }
  ...
  tool(
    ...
    "vitrin_open_studio",
    {
      ...
      description:
        "Vitrin Studio'yu macOS'ta varsayılan tarayıcıda açar — kullanıcı işi elle " +
    ...
    async ({ projectId, page }) => {
      const url =
        page === "headless-export" && projectId
      ...
      await new Promise<void>((resolve, reject) => {
        exec(`open ${JSON.stringify(url)}`, (err) =>
          err ? reject(new VitrinError(`Tarayıcı açılamadı: ${err.message}`, url)) : resolve()
        );
      });
