 ...
 */
async function moveTree(from: string, to: string): Promise<string[]> {
...
export function registerRender(server: Server): void {
  tool(
    server,
    "vitrin_render_preview",
    {
      title: "Slayt önizleme",
      description:
        "Tek slaydı GERÇEK motorla (canvas + WebGL) basar ve görsel olarak döner. " +
    ...
    async ({ projectId, locale, slide, width }, ctx) => {
      ...
      return image(
        out.base64,
        `${p.app} · ${loc} · slayt ${i} · ${out.width}×${out.height}\n` +
  ...
  tool(
    server,
    "vitrin_render_contact_sheet",
    {
      title: "Kontak sayfası",
      description:
        "Bir dilin TÜM slaytlarını TEK ızgara görselinde basar — 10 ayrı görsel yerine 1. " +
    ...
    async ({ projectId, locale, slides, cellWidth, columns }, ctx) => {
      ...
      if (!idx.length) throw new VitrinError(`${loc} setinde slayt yok`, "Önce içe aktar.");
      const progress = progressSender(ctx);
      const sheet = await renderContactSheet(
        projectId,
        loc,
        slides === undefined || slides === "all" ? "all" : idx,
        cellWidth ?? 220,
        columns ?? 5,
        { onProgress: (pr) => progress(pr.done, pr.total, pr.current) }
      );
      ...
      return image(
        sheet.base64,
        `${p.app} · ${loc} · ${sheet.count} slayt · ${sheet.width}×${sheet.height}\n${list}`
      );
    }
  );
  ...
  tool(
    ...
    "vitrin_export",
    {
      title: "Export al",
      description:
        "App Store PNG'lerini basar. Varsayılan hedef ~/Desktop/Vitrin/<app>/<dil>/ " +
    ...
    async ({ projectId, locales, sizes, slides, outDir }, ctx) => {
      ...
      for (const l of locs) resolveSlides(p.sets[l], slides === "all" ? "all" : slides);
      ...
      const rows: { locale: string; written: number; dir: string }[] = [];
      ...
      for (const locale of locs) {
        ...
        const summary = await runExport(
          ...
          locale,
          useSizes,
          slides === undefined || slides === "all" ? "all" : slides,
          {
            onProgress: (pr) =>
              progress(base + pr.done, grandTotal, `${locale} · ${pr.current}`),
          }
        );
        ...
        if (outDir) {
          ...
          if (await isEmptyDir(appDir)) await rm(appDir, { recursive: true, force: true });
          dir = target;
        } else {
          for (const f of summary.files) written.push(path.join(EXPORT_ROOT, p.app, f));
        }
      ...
      return withData(
        text(
          `${p.app} · ${total} PNG yazıldı · boyut: ${useSizes.join(", ")}`,
          "",
          ...rows.map((r) => `${r.locale.padEnd(8)} ${r.written} dosya → ${r.dir} (klasörde ${counts[r.locale]} png)`)
        ),
        { app: p.app, total, sizes: useSizes, locales: rows, files: written.slice(0, 200) }
      );
    }
  );
}
