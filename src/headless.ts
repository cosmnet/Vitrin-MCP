...
export type HeadlessParams = {
  project: string;
  /** "" (proje varsayılanı) | "tr" | "tr,en" | "all" */
  locale?: string;
  sizes?: string[];
  slides?: "all" | number[];
  mode: "export" | "preview";
...
function buildUrl(p: HeadlessParams): string {
 ...
 */
export async function runHeadless<T>(
  ...
  return withPage(async (page) => {
    ...
    try {
      ...
      throw new VitrinError(
        `Render ${Math.round(timeout / 1000)} sn içinde bitmedi` +
          (p ? ` (son durum: ${p.done}/${p.total} · ${p.current})` : ""),
        "Daha az slayt/dil ile dene ya da VITRIN_RENDER_TIMEOUT değerini artır."
      );
    } finally {
      clearInterval(poll);
    }
    ...
    if (state.usage) {
      throw new VitrinError(
        "headless-export sayfası proje parametresi almadı",
        "projectId doğru mu? vitrin_list_projects ile bak."
      );
    }
    if (state.error) {
    ...
    if (state.webgl === false) {
      throw new VitrinError(
        "Tarayıcıda WebGL yok — cihazsız (bozuk) slayt basılacaktı, iş durduruldu",
        "Chrome'u güncelle; MCP zaten SwiftShader'a düşmeyi deniyor."
      );
    }
...
export async function renderPreview(
  ...
  return runHeadless(
    { project: projectId, locale, slides: [slide], mode: "preview", width },
    opts,
    async (page) => {
      ...
      if (!item) throw new VitrinError("Önizleme üretilmedi", "Slayt indeksi doğru mu?");
      return {
...
export async function renderContactSheet(
  ...
  columns: number,
  opts: RunOpts = {}
): Promise<{ base64: string; count: number; width: number; height: number }> {
  return runHeadless(
    { project: projectId, locale, slides, mode: "preview", width: cellWidth },
    opts,
    async (page) => {
      const out = await page.evaluate(async (cols: number) => {
      ...
      if (!out) throw new VitrinError("Kontak sayfası üretilmedi", "Projede slayt var mı?");
      return {
 ...
 */
export async function prepareVisionShots(
  shots: { id: string; name: string; dataUrl: string }[],
  maxH = 760,
  limit = 12
  ...
  return withPage(async (page) => {
    ...
    return page.evaluate(
      async (input: { rows: { id: string; name: string; src: string }[]; maxH: number }) => {
...
export async function runExport(
  ...
  return runHeadless(
    { project: projectId, locale, sizes, slides, mode: "export" },
    opts,
    async (page) => {
      ...
      return summary as ExportSummary;
    }
  );
}
